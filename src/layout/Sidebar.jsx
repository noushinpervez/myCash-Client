...
const Sidebar = () => {
    ...
    const { data: transactionsData = {}, isLoading: transactionLoading } = usePendingTransactions(data?.email, data?.number);
    ...
    return (
        <div className='min-h-screen antialiased text-gray-900 bg-gray-100'>
                ...
                <div className='z-50 flex flex-col flex-1'>
                    <div className='flex items-center justify-between flex-shrink-0 w-[90%] p-4'>
                        <Link to='/' className='flex items-center'>
                            <img src={ logo } className='w-8 md:w-12 h-auto' />
                            <p className='text-blue-500 text-2xl font-semibold cursive'>myCash</p>
                        </Link>
                        <button onClick={ () => setIsSidebarOpen(false) } className='p-1 rounded-md focus:outline-none focus:ring focus:ring-blue-500'>
                            <svg aria-hidden='true' className='w-6 h-6' viewBox='0 0 24 24' fill='none'>
                                <path d='M19 5L5 19M5.00001 5L9.5 9.5M19 19L12 12' stroke='#3b82f6' strokeWidth='2.5' strokeLinecap='round' strokeLinejoin='round' />
                            </svg>
                            <span className='sr-only'>Close sidebar</span>
                        </button>
                    </div>
                    <div className='flex items-center justify-center w-[75%] px-4 my-2'>
                        <div className='w-full bg-blue-50 text-blue-500 border border-blue-400 font-medium px-3 py-0.5 rounded-full flex items-center justify-center gap-2 text-xl'>
                    ...
                    <nav className='flex flex-col flex-1 px-4 mt-4 gap-2 font-medium w-[75%]'>
                        ...
                        { user?.role === 'Admin' &&
                            <NavLink to='/manage-users' className='flex items-center space-x-2 p-3 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out' style={ ({ isActive }) => (isActive ? activeLinkStyle : {}) }>
                                <svg className='w-6 h-6' aria-hidden='true' fill='currentColor' viewBox='0 0 32 32' stroke='currentColor'>
                        ...
                        { user?.role === 'Agent' && data?.status === 'active' ? (
                            <NavLink to='/requests' className='flex items-center space-x-2 p-3 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out' style={ ({ isActive }) => (isActive ? activeLinkStyle : {}) }>
                        ...
                        { data?.status === 'active' &&
                            <NavLink to='/send-money' className='flex items-center space-x-2 p-3 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out' style={ ({ isActive }) => (isActive ? activeLinkStyle : {}) }>
                                <svg className='w-6 h-6' aria-hidden='true' fill='none' viewBox='-0.5 0 25 25' stroke='currentColor' strokeWidth='2'>
                        ...
                        { data?.status === 'active' &&
                            <NavLink to='/statement' className='flex items-center space-x-2 p-3 rounded-md hover:bg-blue-500 hover:text-white transition-all duration-300 ease-in-out' style={ ({ isActive }) => (isActive ? activeLinkStyle : {}) }>
            ...
            <main className='flex flex-col items-center justify-center flex-1'>
                <button onClick={ () => setIsSidebarOpen(true) } className='fixed p-2 text-white bg-blue-600 rounded-md top-5 left-5'>
